...
round_0_log = f"Round 0 Testing Accuracy: {round_0_accuracy:.4f}"
print(round_0_log)
...
for rd in range(1, args.n_round + 1):
    ...
    # Update dataset to include the next set of classes
    ...
    # Decide whether to reinitialize the model or continue training
    # Option 1: Reinitialize the model (uncomment the following lines)
    # net = get_net(args.dataset_name, device)
    # strategy = strategy_class(dataset, net)
    # Option 2: Continue training without reinitialization (current behavior)
    ...
    round_log = f"Round {rd} Testing Accuracy: {round_accuracy:.4f}"
    print(round_log)
    ...
    cm = confusion_matrix(dataset.Y_test, preds, labels=range(num_classes))
    with np.errstate(divide='ignore', invalid='ignore'):
        class_acc = np.true_divide(cm.diagonal(), cm.sum(axis=1))
        class_acc[cm.sum(axis=1) == 0] = 0
    per_class_accuracies.append(class_acc)
